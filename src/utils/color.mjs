/**
 * 颜色解析模块
 * 支持 hex, rgb, rgba, hsl, hsla, CSS named colors
 */

/** CSS named colors (完整列表) */
const NAMED_COLORS = {
...
export function parseColor(color) {
  ...
  if (rgbMatch) {
    ...
    if (rgbMatch[2] === "%") r = Math.round(r * 2.55);
    if (rgbMatch[4] === "%") g = Math.round(g * 2.55);
    if (rgbMatch[6] === "%") b = Math.round(b * 2.55);
