...
function assertMatrixClose(actual, expected, msg) {
...
describe("parseTransform", () => {
  ...
  it("rotate(90) → cos/sin 验证", () => {
    ...
    assertMatrixClose(m, [Math.cos(Math.PI/2), Math.sin(Math.PI/2), -Math.sin(Math.PI/2), Math.cos(Math.PI/2), 0, 0]);
  });
  ...
  it("rotate(45, 100, 100) 带中心点旋转", () => {
    ...
    const expected = [cos, sin, -sin, cos, 100*(1-cos)+100*sin, 100*(1-cos)-100*sin];
    assertMatrixClose(m, expected);
  });

  it("skewX(45)", () => {
    const m = parseTransform("skewX(45)");
    assertMatrixClose(m, [1, 0, Math.tan(Math.PI/4), 1, 0, 0]);
  });

  it("skewY(30)", () => {
    const m = parseTransform("skewY(30)");
    assertMatrixClose(m, [1, Math.tan(Math.PI/6), 0, 1, 0, 0]);
  });
  ...
  it("链式: translate(10,20) scale(2)", () => {
    ...
    assertMatrixClose(m, [2, 0, 0, 2, 10, 20]);
  });
