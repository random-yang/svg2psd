...
import { convertSvg, enrichTextDescriptors, countAllLayers } from "../../src/core/converter-core.mjs";
...
describe("convertSvg", () => {
  it("渲染并返回 psd 对象", async () => {
    ...
    assert.ok(psd);
    assert.equal(psd.width, 100);
    assert.equal(psd.height, 100);
    assert.ok(psd.children.length > 0);
