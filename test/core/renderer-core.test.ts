...
describe("buildStandaloneSvg", () => {
  it("生成包含 defs 的完整 SVG（带 serializeFn）", () => {
    ...
    let rectEl: Element | undefined;
    for (let i = 0; i < children.length; i++) {
      const c = children[i];
      if (c.nodeType === 1 && ((c as Element).localName || c.nodeName) === "rect") {
        rectEl = c as Element;
        break;
      }
    }
  ...
  it("应用变换矩阵", () => {
    ...
    const result = buildStandaloneSvg(rect, svg, [2, 0, 0, 2, 10, 20] as Matrix, serializeFn);
  ...
  it("单位矩阵不添加 transform", () => {
    ...
    const svg = doc.documentElement;
    const rect = svg.getElementsByTagName("rect")[0];
    const result = buildStandaloneSvg(rect, svg, [1, 0, 0, 1, 0, 0] as Matrix, serializeFn);
